    ...
    const fetchReflection = async (targetDate: string) => {
        ...
        try {
            ...
            if (Array.isArray(data)) {
                ...
                reflection = data.reflections.length > 0 ? (data.reflections[0] as Reflection) : null;
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-semibold text-zinc-900">회고</h2>
                    <p className="text-sm text-zinc-600">
                        하루를 돌아보며 느낀 점과 배운 점을 짧게 기록해 보세요.
                    </p>
                </div>
            ...
            <div className="rounded-xl border border-zinc-200 bg-white p-5 space-y-4">
                <div className="space-y-1">
                    <p className="text-sm font-medium text-zinc-800">
                        {date === getTodayString() ? "오늘의 회고" : "선택한 날짜의 회고"}
                    </p>
                    {loading ? (
                        <p className="text-xs text-zinc-500">회고를 불러오는 중입니다...</p>
                    ) : currentReflection ? (
                        ...
                        <p className="text-xs text-zinc-500">
                            아직 회고가 기록되지 않았어요. 오늘 있었던 일이나 느낀 점을 남겨보세요.
                        </p>
                    )}
                </div>
